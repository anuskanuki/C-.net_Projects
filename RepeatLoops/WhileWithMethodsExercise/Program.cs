...
namespace FindingWordsWithForExercise
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Founded: {wordCount}");
            Console.ReadKey();
        ...
        /// <summary>
        /// Find the word using while loop. Search methot to the application
        /// </summary>
        /// <param name="wordToFind"></param>Until 3 characters paramether 
        /// <returns></returns>returns how many was found
        private static int FindByWhile(string wordToFind)
        {
            var contentToFind = TextContent();//the content(text) where we will search for the word
            var wordFindCount = wordToFind.Length;//size to thw searched word 
            var textLength = contentToFind.Length - (wordFindCount - 1);//-2 cause: in the end, the loop stops right in the end, not after already finished
            var contWordFind = 0;
            int i = 0;
